...
# Statuses that mean Circle/Gateway ultimately did NOT settle the transfer,
# even though it may have looked "accepted" earlier. Rare in practice, but
# must be handled: report access may already have been granted before this
# was known. Configurable via env since the exact Circle status vocabulary
# for terminal failures isn't fully documented; adjust if Circle's API uses
...
def invoice_has_failed_settlement(invoice: dict) -> bool:
    """True once report access was already granted (invoice paid / legs paid)
    but Circle later reports a terminal failure on the settlement. This is
    the rare "accepted then finally failed" case. We never revoke the report
    already delivered to the client, but we must stop treating this invoice
    as good for future access, caching, or provider/platform claim."""
...
def refresh_split_leg_batch_txs(invoice: dict) -> bool:
    if invoice_split_mode(invoice) != "x402_direct_split":
        return False
    changed = False
...
def invoice_payment_state_response(
    ...
    *,
    ...
    seller_balance = fetch_gateway_balance(PAYMENT_WALLET_ADDRESS) if include_seller_balance and invoice_split_mode(invoice) != "x402_direct_split" else {}
...
# --- Cross-process locking for payment settlement mutations -----------------
# threading.Lock() only protects against races within a single process. On
# Render (or any horizontally-scaled deployment) multiple worker processes
# can call /payment/verify for the same invoice concurrently, and since
# invoices are persisted as JSON (last-write-wins), a race can silently drop
# an already-verified split leg. We take an OS-level flock on a per-key lock
# file as a second layer, in addition to the in-process split_leg_lock.
try:
    import fcntl
...
_LOCK_DIR = os.path.join(os.path.dirname(__file__), ".locks")
...
def settlement_id_already_claimed(settlement_id: Optional[str], *, exclude_invoice_id: Optional[str] = None) -> bool:
    """True if settlement_id is already recorded against a paid leg on a
    *different* invoice. Prevents the same Circle settlement receipt from
    being replayed across invoices/legs."""
    if not settlement_id:
        return False
    try:
        all_invoices = load_invoices()
    except Exception:
        all_invoices = invoices_db
...
def summarize_payment_events(events: list) -> dict:
    ...
    for event in sorted_events:
        ...
        if split_role == "creator":
            ...
            provider_stats["_creator_earned_final_usdc"] += final_amount
            provider_stats["withdrawal_mode"] = "direct_gateway_split"
        elif split_role == "platform":
            provider_stats["platform_fee_usdc"] += amount
            provider_stats["_platform_fee_final_usdc"] += final_amount
            provider_stats["withdrawal_mode"] = "direct_gateway_split"
        else:
            provider_stats["creator_earned_usdc"] += amount * provider_stats["creator_share_bps"] / 10000
            provider_stats["_creator_earned_final_usdc"] += final_amount * provider_stats["creator_share_bps"] / 10000
            provider_stats["platform_fee_usdc"] += amount * provider_stats["platform_share_bps"] / 10000
            provider_stats["_platform_fee_final_usdc"] += final_amount * provider_stats["platform_share_bps"] / 10000
        ...
        provider_stats["creator_claimable_usdc"] = 0.0 if provider_stats["withdrawal_mode"] == "direct_gateway_split" else max(
            0.0,
            provider_stats["_creator_earned_final_usdc"] - claim_amounts["reserved_usdc"],
        )
        provider_stats["creator_pending_batch_usdc"] = max(
            0.0,
            provider_stats["creator_earned_usdc"] - provider_stats["_creator_earned_final_usdc"],
        )
        provider_stats["platform_pending_batch_usdc"] = max(
            0.0,
            provider_stats["platform_fee_usdc"] - provider_stats["_platform_fee_final_usdc"],
        )
    ...
    for stats in revenue_by_provider.values():
        ...
        stats.pop("_creator_earned_final_usdc", None)
        stats.pop("_platform_fee_final_usdc", None)
        ...
        provider_breakdown.append({
            **stats,
            ...
            "creator_pending_batch_usdc": round(stats["creator_pending_batch_usdc"], 6),
            "platform_fee_usdc": round(stats["platform_fee_usdc"], 6),
            "platform_pending_batch_usdc": round(stats["platform_pending_batch_usdc"], 6),
...
def reconcile_disputed_invoices(max_invoices: int = 20) -> None:
    """Reconciliation worker (best-effort, called from the periodic tick):
    poll Circle for settlements on paid invoices/legs that are not yet in a
    final state, and flip the invoice to `disputed` if Circle reports a
    terminal failure after we already granted report access. We never revoke
    access already delivered to the client — this only stops future access,
    caching, and provider/platform claim for the invoice.
    """
    candidates = []
    for invoice_id, invoice in list(invoices_db.items()):
...
def build_provider_stats(provider_id: str) -> dict:
    ...
    claimable = 0.0 if direct_split else max(0.0, earned_final - claim_amounts["reserved_usdc"])
...
def reserve_internal_split_leg(
    ...
    require_internal_gateway_secret(x_qma_internal_secret)
    with cross_process_lock("split_leg:" + invoice_id):
      with split_leg_lock:
        invoice = invoices_db.get(invoice_id)
        if not invoice:
            raise HTTPException(status_code=404, detail="Invoice not found.")
...
def record_internal_split_leg(
    ...
    with cross_process_lock("split_leg:" + invoice_id):
      with split_leg_lock:
        ...
        if settlement_id_already_claimed(settlement_id, exclude_invoice_id=invoice_id):
...
@app.get("/api/v1/payment/invoices/{invoice_id}/status")
def get_payment_invoice_status(
    ...
    if invoice_split_mode(invoice) == "x402_direct_split":
        ...
        before = (
            ...
            invoice.get("transaction_hash"),
            invoice.get("explorer_url"),
        )
    ...
    if changed:
        save_invoice(invoice)
    return invoice_payment_state_response(invoice_id, invoice, include_access_token=True)
    ...
    with cross_process_lock("split_leg:" + invoice_id):
      with split_leg_lock:
        for leg in required_legs:
            ...
            if not submitted:
                continue
            # Idempotency guard: (invoice_id, leg_id, settlement_id) is the unique key.
            # If this leg is already paid, do NOT re-verify or overwrite it — just no-op
            # if the resubmission matches, or reject if it doesn't.
            if leg.get("status") == "paid" and leg.get("settlement_id"):
                if leg.get("settlement_id") != submitted.settlement_id:
                    raise HTTPException(
                        status_code=409,
                        detail=f"Split leg {leg_id} is already settled with a different settlement_id. Refusing to overwrite.",
                    )
            ...
            # Global cross-invoice guard: the same settlement_id must never be attached
            # to two different legs/invoices (replay across invoices).
            if settlement_id_already_claimed(submitted.settlement_id, exclude_invoice_id=invoice_id):
                raise HTTPException(
                    status_code=409,
                    detail=f"settlement_id for leg {leg_id} is already claimed by another invoice/leg.",
                )
...
def verify_payment(invoice_id: str = Query(...), proof: Optional[PaymentVerifyRequest] = None):
    ...
    if not proof.settlement_id:
        raise HTTPException(status_code=400, detail="settlement_id is required.")
    if settlement_id_already_claimed(proof.settlement_id, exclude_invoice_id=invoice_id):
        raise HTTPException(status_code=409, detail="settlement_id is already claimed by another invoice.")
    with cross_process_lock("split_leg:" + invoice_id):
        # Re-check under the lock: another concurrent request may have just paid this invoice.
        invoice = get_invoice_or_402(invoice_id)
        hydrate_payment_schema(invoice)
        if invoice.get("status") == "paid":
            return invoice_payment_state_response(
...
def authorize_paid_invoice(
    *,
    ...
    if invoice_has_failed_settlement(invoice):
        # Report may already have been delivered once (we cannot revoke a copy
        # the client already has), but we never issue any further access, cached
        # or otherwise, once Circle reports a terminal settlement failure.
        if invoice.get("status") != "disputed":
            invoice["status"] = "disputed"
            save_invoice(invoice)
        raise HTTPException(
            status_code=status.HTTP_402_PAYMENT_REQUIRED,
            detail={
                "error": "settlement_disputed",
                "message": "Circle reported a terminal settlement failure for this invoice after access was granted. No further access will be issued; contact support if you believe this is an error.",
            },
        )
