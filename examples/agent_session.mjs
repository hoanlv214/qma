...
const hasLoopBound = Boolean(durationArg || untilStopped || argValue("max-purchases"));
...
function runPurchase(candidate, state, live) {
  ...
  return new Promise((resolve) => {
    ...
    child.on("close", (code) => resolve(code === 0
      ? { status: "completed", provider_id: candidate.provider_id, symbol: candidate.symbol, tier: candidate.tier, amount_usdc: candidate.price_usdc, access_token_received: true, report_unlocked: true, error: null }
...
const report = await runAutonomousSession(policy, {
  observe: async () => {
    ...
    return {
      candidates: resolved ? [{ ...resolved, owned: false }] : [],
      candidateCount: Number(decision.candidate_count || 0),
      metadata: { decision_source: decision.decision_source, plan: decision.plan, rejected_candidates: decision.rejected_candidates },
    };
  },
